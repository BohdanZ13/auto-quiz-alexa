  ...
  const APP_ID =  "amzn1.ask.skill.3ffa47a3-d4f6-4a77-9741-23866223dba4"; // TODO replace with your app ID (OPTIONAL)
  ...
  const languageString = {
      'de': {
          'translation': {
              ...
              'GAME_NAME': 'Auto Quiz', // Be sure to change this for your skill.
              ...
              'HELP_REPROMPT': 'Wenn Sie eine Frage beantworten wollen, antworte mit der Zahl, die zur richtigen Antwort gehört. ',
              'STOP_MESSAGE': 'Möchten Sie weiterspielen?',
              ...
              'WELCOME_MESSAGE': ' <break time="0.5s"/> Ich stelle dir %s Fragen und du versuchst, so viele wie möglich richtig zu beantworten. <break time="0.5s"/>' +
              'Sage einfach die Zahl, die zur richtigen Antwort passt. Fangen wir an. ',
              'ANSWER_CORRECT_MESSAGE': ' <break time="0.5s"/> Richtig.  <break time="0.5s"/>',
              'ANSWER_WRONG_MESSAGE': '<break time="0.5s"/> Falsch.  <break time="0.5s"/>',
              'CORRECT_ANSWER_MESSAGE': '<break time="0.5s"/> Die richtige Antwort ist %s: %s. <break time="0.5s"/>',
              'ANSWER_IS_MESSAGE': 'Diese Antwort ist ',
              'TELL_QUESTION_MESSAGE': '<break time="0.5s"/>  Frage %s. <break time="0.5s"/> %s  <break time="0.5s"/>',
              'GAME_OVER_MESSAGE': '<break time="0.5s"/> Du hast %s von %s richtig beantwortet. Danke fürs Mitspielen! <break time="0.5s"/>',
              'SCORE_IS_MESSAGE': ' <break time="0.5s"/> ',
              //'SCORE_IS_MESSAGE': ' <break time="0.5s"/> Dein Ergebnis ist %s. <break time="0.5s"/>',
          },
      },
  };
  ...
  function isAnswerSlotValid(intent) {
      ...
      var valueNum = getNum(answerSlotFilled, intent);
      const answerSlotIsString = answerSlotFilled && !isNaN(parseInt(valueNum, 10));
      return answerSlotIsString
          && parseInt(valueNum, 10) < (ANSWER_COUNT + 1)
          && parseInt(valueNum, 10) > 0;
  }
  function getNum(answerSlotFilled, intent){
    console.log(answerSlotFilled +" answer "+ intent.slots.Answer.value);
    var valueNum = '';
        if (answerSlotFilled) {
        switch(intent.slots.Answer.value) {
        case 'a':  // if (x === 'value1')
        valueNum ="1";
        break;
        case 'b':  // if (x === 'value2')
  ...
  function handleUserGuess(userGaveUp) {
      ...
      var valueNum = getNum(true, this.event.request.intent);
      console.log("answer: " + valueNum + " CORRECT IS: " + this.attributes['correctAnswerIndex']);
      if (answerSlotValid && parseInt(valueNum, 10) === this.attributes['correctAnswerIndex']) {
          currentScore++;
      ...
      if (this.attributes['currentQuestionIndex'] === GAME_LENGTH - 1) {
          ...
          for (let i = 0; i < ANSWER_COUNT; i++) {
            ...
            repromptText += `${caseLetter}.<break time="0.5s"/> ${roundAnswers[i]}. `;          }
